...
include "PHPExcel/IOFactory.php";
				echo $getFileId = $_GET['fileid'];
				echo $getFormatId = $_GET['formatid'];
				echo $getFilePath = $_GET['filepath'];
					$seller_id = $_GET['sellerid'];
				
			/* $excel_name = $_FILES['excel-upload']['name'];*/
			 $fileType = pathinfo($getFilePath,PATHINFO_EXTENSION);
			...
			$objPHPExcel = PHPExcel_IOFactory::load($getFilePath);
			$t=time();
			if($getFormatId == 1){
			...
			foreach ($objPHPExcel -> getWorksheetIterator() as $worksheet) {
				...
				for($row=2;$row<=$highestRow;$row++){
					...
					$updateFileStatus = "UPDATE `oc_seller_excel` SET `status`= 1 WHERE `id` = '".$getFileId."' ";
					if(!$checkUpdateFileStatus = mysqli_query($con,$updateFileStatus)){
						mysqli_error($con);
						mysqli_close($con);
